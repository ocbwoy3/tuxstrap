...
export class BloxstrapRPC {
	...
	constructor(aw: ActivityWatcher) {
		...
		this.rp.on("ready", () => {
			console.log(
				"[BloxstrapRPC]",
				`Connected to Discord as ${this.rp?.user?.username}#${this.rp?.user?.discriminator} - ${this.rp?.user?.id}`
			);
		});
		...
		this.rp
			.login({ clientId: constants.DISCORD_APPID })
			.catch(console.error);
	}
	...
	public async setStashedMessage(rp: RichPresence) {
		...
		if (rp.timeStart)
			this._stashedRPCMessage.timeStart =
				<any>rp.timeStart === 0 ? undefined : rp.timeStart;
		if (rp.timeEnd)
			this._stashedRPCMessage.timeStart =
				<any>rp.timeEnd === 0 ? undefined : rp.timeEnd;
		if (rp.details)
			this._stashedRPCMessage.details =
				rp.details.length === 0 ? undefined : rp.details;
		if (rp.state)
			this._stashedRPCMessage.state =
				rp.state.length === 0 ? undefined : rp.state;

		if (rp.largeImage) {
			if (!this._stashedRPCMessage.largeImage)
				this._stashedRPCMessage.largeImage = {};
			if (rp.largeImage.assetId)
				this._stashedRPCMessage.largeImage.assetId =
					rp.largeImage.assetId;
			if (rp.largeImage.hoverText)
				this._stashedRPCMessage.largeImage.hoverText =
					rp.largeImage.hoverText;
			if (rp.largeImage.clear)
				this._stashedRPCMessage.largeImage.hoverText = undefined;
			if (rp.largeImage.reset) this._stashedRPCMessage.largeImage = {};
		}
		if (rp.smallImage) {
			if (!this._stashedRPCMessage.smallImage)
				this._stashedRPCMessage.smallImage = {};
			if (rp.smallImage.assetId)
				this._stashedRPCMessage.smallImage.assetId =
					rp.smallImage.assetId;
			if (rp.smallImage.hoverText)
				this._stashedRPCMessage.smallImage.hoverText =
					rp.smallImage.hoverText;
			if (rp.smallImage.clear)
				this._stashedRPCMessage.smallImage.hoverText = undefined;
	...
	public async updateRichPresence() {
		...
		try {
			...
			console.log(
				"[BloxstrapRPC]",
				`Could not get Universe ID! PlaceId: ${this.aw.ActivityPlaceId}`
			);
			return false;
		}
		...
		try {
			universeDetails = (await GetPlaceDetails(
				universeId
			...
			console.log(
				"[BloxstrapRPC]",
				`Could not get Universe details! PlaceId: ${this.aw.ActivityPlaceId} UniverseId: ${universeId}`
			);
			return false;
		}

		if (universeDetails.name.length < 2)
			universeDetails.name = `${universeDetails.name}\xE2\xE2\xE2`;
		...
		try {
			...
			console.log(
				"[BloxstrapRPC]",
				`Could not get Universe icon! PlaceId: ${this.aw.ActivityPlaceId} UniverseId: ${universeId}`
			);
			return false;
		}
		...
		let status =
			"by " +
			universeDetails.creator.name +
			(universeDetails.creator.hasVerifiedBadge ? " ☑️" : "");
		...
		try {
			let rpc: Presence = {
				details: this._stashedRPCMessage?.details
					? this._stashedRPCMessage.details
					: universeDetails.name,
				state: this._stashedRPCMessage?.state
					? this._stashedRPCMessage.state
					: status,
				startTimestamp: this._stashedRPCMessage?.timeStart,
				endTimestamp: this._stashedRPCMessage?.timeEnd,
				largeImageKey: this._stashedRPCMessage?.largeImage?.assetId
					? `https://assetdelivery.roblox.com/v1/asset/?id=${this._stashedRPCMessage.largeImage.assetId}`
					: thumbnailIcon,
				largeImageText: this._stashedRPCMessage?.largeImage?.hoverText,
				smallImageKey: this._stashedRPCMessage?.smallImage?.assetId
					? `https://assetdelivery.roblox.com/v1/asset/?id=${this._stashedRPCMessage.smallImage.assetId}`
					: constants.SMALL_IMAGE_KEY,
				smallImageText: this._stashedRPCMessage?.smallImage?.hoverText,
				buttons: [
					{
						label: "See game page",
						url: `https://www.roblox.com/games/${this.aw.ActivityPlaceId}`,
					},
				],
			};
