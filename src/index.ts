...
import { _libocbwoy3Version, libocbwoy3Greet, setConsoleTitle } from "@ocbwoy3/libocbwoy3";
...
(()=>{
	...
	const listPluginsSwitch = process.argv.find(
		(a) => a === "--list-plugins"
	);

	const helpSwitch = process.argv.find(
		(a) => (a === "-h") || (a === "--help")
	);

	if (helpSwitch) {
		console.log(`TuxStrap - Basically Bloxstrap for Linux!
		...
		if (
			...
			console.log(
				`TuxStrap ${TUXSTRAP_VERSION}, libocbwoy3 ${_libocbwoy3Version}, Bun ${Bun.version_with_sha}, ${process.argv0.includes("/nix/store") ? "Nix build" : "compiled"}`
			);
		}
		process.exit(0);
	}
	...
	if (listPluginsSwitch) {
		for (const plugin of getPluginInfos()) {
			console.log(`${plugin.name} (${plugin.id})${plugin.forceEnable ? " [default]" : ""}`)
		}
		process.exit(0);
	}
...
(async () => {
	...
	const pluginsEnable = process.argv.filter(
		(a) => a.startsWith("+") && !a.includes("-") && !a.includes(" ")
	).map(a=>a.replace(/^\+/,""));
	const pluginsDisable = process.argv.filter(
		(a) => a.startsWith("-") && !a.startsWith("--") && !a.includes("+") && !a.includes(" ")
	).map(a=>a.replace(/^\-\-/,""));
